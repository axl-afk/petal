  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final tabs = Wrap(
      ...
      children: LibraryTab.values.map((tab) {
        final isActiveTab = section == AppSection.library && libraryState.tab == tab && !libraryState.filter.isActive;
        return _TabButton(
          label: _labelFor(tab),
          active: isActiveTab,
          onTap: () {
    ...
    final actions = Row(
      ...
      children: [
        IconButton(
          ...
          color: section == AppSection.addSource ? petal.colors.ink : petal.colors.ink2,
          onPressed: () => ref.read(currentSectionProvider.notifier).state = AppSection.addSource,
        ),
        IconButton(
          tooltip: 'Settings',
          icon: const Icon(Icons.settings_outlined, size: 20),
          color: section == AppSection.settings ? petal.colors.ink : petal.colors.ink2,
          onPressed: () => ref.read(currentSectionProvider.notifier).state = AppSection.settings,
        ),
        GestureDetector(
          onTap: () => ref.read(currentSectionProvider.notifier).state = AppSection.settings,
          child: CircleAvatar(
            radius: 15,
            ...
            child: session == null
                ...
                : Text(
                    ...
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: petal.colors.ink),
                  ),
          ),
        ),
      ],
    ...
    final container = Container(
      ...
      child: isMobile
          ? Column(
              ...
              children: [
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [brand, actions]),
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Text(label, style: active ? petal.text.tabLabelActive : petal.text.tabLabel),
        ),
      ),
    );
  }
